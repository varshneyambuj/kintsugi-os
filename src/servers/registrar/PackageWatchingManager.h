...
/** @file PackageWatchingManager.h
 *  @brief Manages watchers that monitor package installation and removal events. */
...
/** @brief Registers watchers for package events and broadcasts notifications to them. */
class PackageWatchingManager {
public:
								PackageWatchingManager();
								~PackageWatchingManager();

			/** @brief Processes requests to start or stop watching package events. */
			void				HandleStartStopWatching(BMessage* request);
			/** @brief Sends a package event notification to all registered watchers. */

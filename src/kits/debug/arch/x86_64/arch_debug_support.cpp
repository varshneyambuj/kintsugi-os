 ...
 * @brief Architecture-specific debug support for x86-64 (AMD64).
 *
 * Provides x86-64 implementations of the architecture-dependent debug helper
 * functions used by the debug kit. The instruction pointer is obtained from
 * cpuState.rip and the stack frame base from cpuState.rbp, following the
...
/** @brief Standard two-word stack frame layout used on x86-64. */
struct stack_frame {
 ...
 * then reads the instruction pointer from cpuState.rip and the frame pointer
 * from cpuState.rbp.
 *
 ...
 * @param ip                Output — receives the current instruction pointer (rip).
 * @param stackFrameAddress Output — receives the base pointer address (rbp).
 ...
 * @a stackFrameAddress (System V AMD64 ABI layout: [rbp, return_address]) and
 * fills in @a stackFrameInfo accordingly.
 *
 * @param context            The debug context identifying the target team.

 ...
 * @brief Architecture-specific debug support stubs for ARM.
 *
 * Provides ARM 32-bit implementations of the architecture-dependent debug
 * helper functions used by the debug kit. Both functions currently return
 * B_ERROR as this architecture's register access is not yet implemented.
 *
 ...
 *
 * Queries the CPU state of the specified thread through the debug nub port and
 * extracts the program counter and frame pointer. On ARM this is not yet
 * implemented.
 *
 ...
 * @a stackFrameAddress and fills in @a stackFrameInfo accordingly. On ARM

 ...
 * @brief Architecture-specific debug support for SPARC.
 *
 * Provides SPARC implementations of the architecture-dependent debug helper
 * functions used by the debug kit. The instruction pointer is read from
 * cpuState.pc and the stack frame address from the i6 (frame pointer) register.
 *
...
/** @brief Standard two-word stack frame layout used on SPARC. */
struct stack_frame {
 ...
 * the i6 register (SPARC frame pointer convention).
 *
 ...
 * @param pc                Output — receives the current program counter.
 ...
 * @a stackFrameAddress (SPARC ABI convention) and fills in @a stackFrameInfo
 * accordingly.
 *
 * @param context            The debug context identifying the target team.

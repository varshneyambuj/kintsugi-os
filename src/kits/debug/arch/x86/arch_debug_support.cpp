 ...
 * @brief Architecture-specific debug support for x86 (IA-32).
 *
 * Provides x86 32-bit implementations of the architecture-dependent debug
 * helper functions used by the debug kit. The instruction pointer is obtained
 * from cpuState.eip and the stack frame base from cpuState.ebp, following the
 * standard x86 frame layout.
 *
...
/** @brief Standard two-word stack frame layout used on x86. */
struct stack_frame {
 ...
 * then reads the instruction pointer from cpuState.eip and the frame pointer
 * from cpuState.ebp.
 *
 ...
 * @param ip                Output — receives the current instruction pointer (eip).
 * @param stackFrameAddress Output — receives the base pointer address (ebp).
 ...
 *
 * Reads the saved frame pointer and return address from the memory at
 * @a stackFrameAddress (standard x86 frame layout: [ebp, return_address])
 * and fills in @a stackFrameInfo accordingly.
 *
 * @param context            The debug context identifying the target team.

...
/**
 * @file QueryPoseView.cpp
 * @brief BQueryPoseView — live-query BPoseView that shows BQuery results.
 *
 * Subscribes to live BQuery objects on potentially multiple volumes and
 * populates the pose list as results arrive via node-monitor notifications.
 * Uses a read-only background tint to indicate the view is not writable.
 * Provides default columns (Name, Location, Size, Modified) for new query windows.
 *
 * @see BQueryContainerWindow, BPoseView
 */
...
/**
 * @brief Construct a BQueryPoseView for the given query model.
 *
 * Installs a QueryRefFilter so poses are filtered to exclude certain paths
 * (e.g., the Trash when filtering is enabled).
 *
...
/**
 * @brief Destroy the BQueryPoseView, releasing the ref filter and query container.
 */
...
/**
 * @brief Handle kFSClipboardChanges by always refreshing pose clipboard modes.
 *
 * Query views must refresh on every clipboard change because items may appear
...
/**
 * @brief Apply a read-only tint to the document background to indicate non-writable content.
 */
void
...
/**
 * @brief Return true if the view currently uses the expected read-only system colours.
 *
 * @return True when all three colour roles have the expected values and tints.
 */
bool
...
/**
 * @brief Open the query editor for this query's model entry.
 *
 * Sends a kEditQuery message with the query's entry_ref to the Tracker
 * application so the Find panel is shown with the current query loaded.
 */
void
...
/**
 * @brief Add the standard query columns (Name, Location, Size, Modified) if none exist.
 */
void

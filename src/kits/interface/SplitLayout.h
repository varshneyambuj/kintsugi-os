...
/** @file SplitLayout.h
    @brief Layout backend for BSplitView; arranges items along a single axis with
           draggable splitters between them. */
...
/**
 * @brief Layout that arranges children in a horizontal or vertical strip with
 *        user-draggable splitters between them.
 *
 * BSplitLayout drives BSplitView. Items receive weights that determine how
 * they share space, and may be marked collapsible so a splitter drag can hide
 * a neighbour entirely. Internally a pair of Layouter / LayoutInfo objects
 * compute geometry along the primary and secondary axes; height-for-width
 * children get a separate layouter so the width-dependent secondary axis
 * stays consistent.
 */
